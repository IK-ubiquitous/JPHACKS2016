...
def sound_classiffer():
    mfccs = []
    labels = []
    
    #ファイルの名前位置
    fnames = ['kankyou','warning','horn','siren',]
    target = './class.wav'
    ...
    for fname in fnames:
        for cnt in range(1,MAX_FEATURE_SIZE+1):
            if fcnt == 1:
                fs, data = read('../wav/kankyou/'+fname+str(cnt)+'.wav','rb')
            ...
            if fcnt == 2:
                ...
                fs, data = read('../wav/horn/'+fname+str(cnt)+'.wav','rb')
                mfcc_tmp = calc_mfcc(data, fs)
                if mfcc_tmp[0] != float('inf') and mfcc_tmp[0] != float('-inf'):
                    if mfcc_tmp[0] == mfcc_tmp[0]:
                        mfccs.append(mfcc_tmp)
    ...
    #SVMによる機械学習
    estimator = SVC(C=0.8,kernel='rbf')
