...
class MULTI_SCW1(ConfidenceWeightedModel):
    """
    ...
    def __init__(self, C=1, eta=0.9, epochs=10):
        ...
        """
        super(MULTI_SCW1, self).__init__(epochs)
    ...
    def predict(self, sample):
        ...
        for x in sample:
            if self.data["sparse"]:
                labels.append(self._predict_for_sparse_sample(x))
            else:
                labels.append(self._predict_for_dense_sample(x))
        return labels
...
def main():
    ...
    # learn
    model = MULTI_SCW1(C=1, eta=0.9, epochs=1)
    model.learn(X, y)
